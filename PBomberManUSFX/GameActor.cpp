...
void GameActor::render2()
{
	if (visible) {
		SDL_Rect* cuadradoAnimacion = new SDL_Rect({ imagenX, imagenY, getAncho(), getAlto() });
		textura->render(getPosicionX(), getPosicionY(), cuadradoAnimacion);
	}
}
...
bool GameActor::tratarDeMover(MoveDirection _direccionNueva) {
	...
	if (tileDestino->getMuroSpace() != nullptr)
	{
		setTileSiguiente(nullptr);
		return false;
	}

	if (tileDestino->getMuroHielo() != nullptr)
	{
		setTileSiguiente(nullptr);
		return false;
	}
